...
%Godunov scheme
cell_width = 10; %Width of each cell in x, space step, now it's 100 cell, each is 10 m
delta_x = cell_width;
cell_x = 100; %Number of cells
delta_t = 0.3; %Minimal time step that satisifies CFL condition
time_scale = 60; %Width of time
cell_t = floor( time_scale / delta_t); %Number of cells in time
...
for t = 2:cell_t
    for x = 1:cell_x
        ...
        s1 = downstream_boundary2(t*delta_t);
        s1_tmp = supply(k_t_1_x,kc1,kc2,v,w,kmax);
        s1 = min(s1,s1_tmp);
        ...
        s2 = downstream_boundary2(t*delta_t);
        s2_tmp = supply(k_t_1_x_p_1,kc1,kc2,v,w,kmax);
        s2 = min(s2,s2_tmp);
...
plot(k);
colormap('hot');
title('Godunov Scheme with initial condition , upstream boundary demand and 0 downstream boundary supply');
xlabel('Space (x)');
ylabel('Time (t)');
saveas(gcf,'G4.png')

function dpd = downstream_boundary2(t)
    dpd = 0;
end
